    ...
    public class UsuariosController : ControllerBase
    {
        ...
        public async Task<ActionResult> RegistrarUsuario(RegistroViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (await _usuarioRepository.BuscarQuantidadeUsuariosRegistrados() > 0)
                {
        ...
        public async Task<ActionResult> LogarUsuario(LoginViewModel model)
        {
            ...
            if (usuario != null)
            {
                ...
                {
                    var funcoesUsuario = await _usuarioRepository.BuscarFuncoesUsuario(usuario);
        ...
        {
            Usuario usuario = await _usuarioRepository.BuscarPorId(usuarioId);
        ...
        public async Task<ActionResult> AtualizarUsuario(AtualizarUsuarioViewModel model)
        {
            if (ModelState.IsValid)
            {
                Usuario usuario = await _usuarioRepository.BuscarPorId(model.Id);
